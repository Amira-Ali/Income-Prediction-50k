...
if (length(plots) > 0) {
  print(wrap_plots(plots, ncol = min(3, length(plots))))
}
...
# Safe sourcing inside Rmd chunk
if (!file.exists(file.path(output_path, "data_processing.R"))) {
  stop("data_processing.R not found. Please ensure it is downloaded to output_path")
}

source(file.path(output_path, "data_processing.R"), local = knitr::knit_global())
